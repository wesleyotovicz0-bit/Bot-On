...
def carregar_config() -> dict:
    ...
    data = db.get_document("automations_ai_moderator") or {}
    ...
    if "prompt" not in data:
        data["prompt"] = None
    if "rejection_message" not in data:
        data["rejection_message"] = "violar regras internas."
    return data

def salvar_config(data: dict) -> None:
    """Salva a configuração no banco de dados."""
    # Garante que apenas as chaves esperadas sejam salvas
    config_atual = carregar_config()
    config_atual.update(data)
    db.save_document("automations_ai_moderator", {}, config_atual)
