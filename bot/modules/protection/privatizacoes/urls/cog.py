...
class PrivatizacaoURLsCog(commands.Cog):
    ...
    def _get_punishment_options(punicao_atual: str) -> list[disnake.SelectOption]:
        return [
            disnake.SelectOption(label="Banimento", value="ban", default=punicao_atual == 'ban', description="Bane o membro que enviar uma url."),
            disnake.SelectOption(label="Expulsão", value="kick", default=punicao_atual == 'kick', description="Expulsa o membro que enviar uma url."),
    ...
    def PunishmentPanelEmbed(self, inter: disnake.MessageInteraction):
        ...
        avancado = config.get("privatizacao_urls_avancado", {})
        punicao_atual = avancado.get("punicao", "kick")
        ...
        embed = disnake.Embed(
            ...
            description="Escolha a punição para quem enviar um link não permitido.",
            color=primary_color
        )

        options = self._get_punishment_options(punicao_atual)
        components = [
            disnake.ui.ActionRow(disnake.ui.Select(options=options, custom_id="ProtPrivURLsPunishmentSelect")),
            disnake.ui.ActionRow(disnake.ui.Button(label="Voltar", emoji=emoji.back, custom_id="ProtPrivURLs_Back"))
        ]
    ...
    def PunishmentPanelComponents(self, inter: disnake.MessageInteraction) -> list:
        ...
        return [
            disnake.ui.Container(
                disnake.ui.TextDisplay(f"# {emoji.z0}{emoji.z1}{emoji.z2}{emoji.z3}{emoji.z4}\n-# Painel > Proteção > Privatizações > URLs > **Punição**"),
    ...
    def ImmuneRolePanelEmbed(self, inter: disnake.MessageInteraction):
        ...
        embed = disnake.Embed(
            ...
            description="Selecione os cargos que poderão enviar links.",
            color=primary_color
        )
        ...
        components = [
            disnake.ui.ActionRow(disnake.ui.RoleSelect(custom_id="ProtPrivURLsImmuneRoleSelect", max_values=25)),
            disnake.ui.ActionRow(
                disnake.ui.Button(label="Voltar", emoji=emoji.back, custom_id="ProtPrivURLs_Back"),
                disnake.ui.Button(label="Remover", emoji=emoji.delete, style=disnake.ButtonStyle.red, custom_id="ProtPrivURLsImmuneRoleClear", disabled=not cargos_imunes)
            )
        ]
        return embed, components
    ...
    def ImmuneRolePanelComponents(self, inter: disnake.MessageInteraction) -> list:
        ...
        return [
            disnake.ui.Container(
                disnake.ui.TextDisplay(f"# {emoji.z0}{emoji.z1}{emoji.z2}{emoji.z3}{emoji.z4}\n-# Painel > Proteção > Privatizações > URLs > **Cargos Imunes**"),
                disnake.ui.Separator(),
    ...
    def LogChannelPanelEmbed(self, inter: disnake.MessageInteraction):
        ...
        embed = disnake.Embed(
            ...
            description="Selecione o canal para os logs da proteção de URLs.",
            color=primary_color
        )
        ...
        components = [
            disnake.ui.ActionRow(disnake.ui.ChannelSelect(custom_id="ProtPrivURLsLogChannelSelect", channel_types=[disnake.ChannelType.text])),
            disnake.ui.ActionRow(
                disnake.ui.Button(label="Voltar", emoji=emoji.back, custom_id="ProtPrivURLs_Back"),
                disnake.ui.Button(label="Remover", emoji=emoji.delete, style=disnake.ButtonStyle.red, custom_id="ProtPrivURLsLogChannelClear", disabled=not canal_logs_id),
                disnake.ui.Button(label="Criar para mim", style=disnake.ButtonStyle.blurple, emoji=emoji.wand, custom_id="ProtPrivURLsLogChannelCreate"),
            )
        ]
        return embed, components
    ...
    def LogChannelPanelComponents(self, inter: disnake.MessageInteraction) -> list:
        ...
        return [
            disnake.ui.Container(
                disnake.ui.TextDisplay(f"# {emoji.z0}{emoji.z1}{emoji.z2}{emoji.z3}{emoji.z4}\n-# Painel > Proteção > Privatizações > URLs > **Logs**"),
                disnake.ui.Separator(),
        ...
        components = [
            disnake.ui.ActionRow(
                disnake.ui.Select(
                    ...
                    custom_id="ProtPrivURLsConfigSelect"
                ),
            ),
    ...
    def PainelComponents(self, inter: disnake.MessageInteraction) -> list:
        ...
        return [
            disnake.ui.Container(
                disnake.ui.TextDisplay(f"# {emoji.z0}{emoji.z1}{emoji.z2}{emoji.z3}{emoji.z4}\n-# Painel > Proteção > Privatizações > **URLs**"),
                disnake.ui.Separator(),
    ...
    async def priv_urls_dropdown_listener(self, inter: disnake.MessageInteraction):
        custom_id = inter.component.custom_id
        if "ProtPrivURLs" not in custom_id:
            return
        
        if custom_id == "ProtPrivURLsPunishmentSelect":
            await interactions.handle_punishment_select(self, inter)
            return
        if custom_id == "ProtPrivURLsImmuneRoleSelect":
            await interactions.handle_immune_role_select(self, inter)
            return
        if custom_id == "ProtPrivURLsLogChannelSelect":
    ...
    async def priv_urls_button_listener(self, inter: disnake.MessageInteraction):
        custom_id = inter.component.custom_id
        if "ProtPrivURLs" not in custom_id:
            return

        action_map = {
            "ProtPrivURLs_Back": self.display_panel,
            "ProtPrivURLsImmuneRoleClear": interactions.handle_immune_role_clear,
            "ProtPrivURLsLogChannelClear": interactions.handle_log_channel_clear,
            "ProtPrivURLsLogChannelCreate": interactions.handle_log_channel_create,
        }
        handler = action_map.get(custom_id)
        if handler:
            if custom_id == "ProtPrivURLs_Back":
                await handler(inter)
            else:
                await handler(self, inter)

def setup(bot: commands.Bot):
    bot.add_cog(PrivatizacaoURLsCog(bot))
