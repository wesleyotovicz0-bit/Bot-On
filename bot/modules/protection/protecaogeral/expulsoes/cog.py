...
class ExpulsoesCog(commands.Cog):
    ...
    def PunishmentPanelEmbed(self, inter: disnake.MessageInteraction):
        ...
        avancado = config.get("expulsoes_avancado", {})
        punicao_atual = avancado.get("punicao", "ban")
        ...
        embed = disnake.Embed(
            ...
            description="Escolha a punição para quem atingir o limite de expulsões.",
            color=primary_color
        )

        options = self._get_punishment_options(punicao_atual)
        components = [
            disnake.ui.ActionRow(disnake.ui.Select(options=options, custom_id="ProtExpulsoesPunishmentSelect")),
            disnake.ui.ActionRow(disnake.ui.Button(label="Voltar", emoji=emoji.back, custom_id="ProtExpulsoes_Back"))
        ]
    ...
    def PunishmentPanelComponents(self, inter: disnake.MessageInteraction) -> list:
        ...
        return [
            disnake.ui.Container(
                disnake.ui.TextDisplay(f"# {emoji.z0}{emoji.z1}{emoji.z2}{emoji.z3}{emoji.z4}\n-# Painel > Proteção > Expulsões > **Punição**"),
    ...
    def ImmuneRolePanelEmbed(self, inter: disnake.MessageInteraction):
        ...
        embed = disnake.Embed(
            ...
            description="Selecione os cargos imunes à proteção.",
            color=primary_color
        )
        ...
        components = [
            disnake.ui.ActionRow(disnake.ui.RoleSelect(custom_id="ProtExpulsoesImmuneRoleSelect", max_values=25)),
            disnake.ui.ActionRow(
                disnake.ui.Button(label="Voltar", emoji=emoji.back, custom_id="ProtExpulsoes_Back"),
                disnake.ui.Button(label="Remover", emoji=emoji.delete, style=disnake.ButtonStyle.red, custom_id="ProtExpulsoesImmuneRoleClear", disabled=not cargos_imunes)
            )
        ]
        return embed, components
    ...
    def ImmuneRolePanelComponents(self, inter: disnake.MessageInteraction) -> list:
        ...
        return [
            disnake.ui.Container(
                disnake.ui.TextDisplay(f"# {emoji.z0}{emoji.z1}{emoji.z2}{emoji.z3}{emoji.z4}\n-# Painel > Proteção > Expulsões > **Cargos Imunes**"),
                disnake.ui.Separator(),
    ...
    def LogChannelPanelEmbed(self, inter: disnake.MessageInteraction):
        ...
        components = [
            disnake.ui.ActionRow(disnake.ui.ChannelSelect(custom_id="ProtExpulsoesLogChannelSelect", channel_types=[disnake.ChannelType.text])),
            disnake.ui.ActionRow(
                disnake.ui.Button(label="Voltar", emoji=emoji.back, custom_id="ProtExpulsoes_Back"),
                disnake.ui.Button(label="Remover", emoji=emoji.delete, style=disnake.ButtonStyle.red, custom_id="ProtExpulsoesLogChannelClear", disabled=not canal_logs_id),
                disnake.ui.Button(label="Criar para mim", style=disnake.ButtonStyle.blurple, emoji=emoji.wand, custom_id="ProtExpulsoesLogChannelCreate"),
            )
        ]
        return embed, components
    ...
    def LogChannelPanelComponents(self, inter: disnake.MessageInteraction) -> list:
        ...
        return [
            disnake.ui.Container(
                disnake.ui.TextDisplay(f"# {emoji.z0}{emoji.z1}{emoji.z2}{emoji.z3}{emoji.z4}\n-# Painel > Proteção > Expulsões > **Logs**"),
                disnake.ui.Separator(),
            ...
            disnake.ui.ActionRow(
                disnake.ui.Select(
                    ...
                    options=[
                        ...
                        disnake.SelectOption(label="Definir Limite", value="limite", emoji=emoji.chart, description="Limite de ações por intervalo"),
                        disnake.SelectOption(label="Definir Intervalo", value="intervalo", emoji=emoji.clock, description="Intervalo de tempo para a punição"),
                    ],
                    custom_id="ProtExpulsoesConfigSelect"
                ),
            ),
            disnake.ui.ActionRow(
                disnake.ui.Select(
                    ...
                    custom_id="ProtExpulsoesConfigSelectAvancado"
                ),
            ),
    ...
    async def expulsoes_dropdown_listener(self, inter: disnake.MessageInteraction):
        custom_id = inter.component.custom_id
        if "ProtExpulsoes" not in custom_id:
            return
        
        if custom_id == "ProtExpulsoesPunishmentSelect":
            await interactions.handle_punishment_select(self, inter)
            return
        if custom_id == "ProtExpulsoesImmuneRoleSelect":
            await interactions.handle_immune_role_select(self, inter)
            return
        if custom_id == "ProtExpulsoesLogChannelSelect":
    ...
    async def expulsoes_button_listener(self, inter: disnake.MessageInteraction):
        custom_id = inter.component.custom_id
        if "ProtExpulsoes" not in custom_id:
            return

        action_map = {
            "ProtExpulsoes_Back": self.display_panel,
            "ProtExpulsoesImmuneRoleClear": interactions.handle_immune_role_clear,
            "ProtExpulsoesLogChannelClear": interactions.handle_log_channel_clear,
            "ProtExpulsoesLogChannelCreate": interactions.handle_log_channel_create,
        }
        handler = action_map.get(custom_id)
        if handler:
            if custom_id == "ProtExpulsoes_Back":
                await handler(inter)
            else:
                await handler(self, inter)

def setup(bot: commands.Bot):
    bot.add_cog(ExpulsoesCog(bot))
