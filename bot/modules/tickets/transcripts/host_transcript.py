...
Integra em 3 pontos:
1. Ao fechar um ticket (envia para logs)
2. Ao enviar na DM (envia para logs)
3. Ao clicar no botão (envia para DM ou logs)
"""
            ...
            async with session.post(
                ...
                if resp.status == 200:
                    ...
                    if result.get("success"):
                        # Usar fullUrl se disponível, senão construir
                    ...
                    print(f"[TRANSCRIPT] Erro ao fazer upload: Status {resp.status} - {error_text}")
                    return None
    except Exception as e:
...
async def log_transcript(
    ...
    Usado quando o ticket é fechado e precisa enviar para os logs.
    ...
    :param message_to_reply: Mensagem para responder (log de fechamento)
    ...
    try:
        ...
        if transcript_url and message_to_reply:
            ...
            try:
                ...
                print(f"[TRANSCRIPT] Embed enviado com sucesso nos logs")
            except Exception as e:
            ...
            if message_to_reply:
                try:
                    await message_to_reply.reply(
                        "⚠️ Não foi possível hospedar o transcript na API. Tente novamente mais tarde."
                    )
                except Exception:
                    pass
